 ...
 */
class Simplify_Validation_DateTimeValidator extends Simplify_Validation_AbstractValidator
{
  ...
  {
    if (empty($value)) $this->fail();
    if ($value instanceof DateTime && ! preg_match('/\d{4}-\d{1,2}-\d{1,2}/', $value->format('Y-m-d'))) $this->fail();
  }
