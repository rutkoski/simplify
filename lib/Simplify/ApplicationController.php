 ...
 *
 */
class Simplify_ApplicationController
{
  ...
  public function factory($params)
  {
    ...
    if ($module) {
      $base = s::config()->get('modules:' . $module . ':dir', s::config()->get('modules_dir') . '/' . $module,
        Simplify_Dictionary::FILTER_NULL);

      $filename = $base . '/controller' . $path . '/' . $filename;
    }
    else {
      $base = s::config()->get('app_dir');
